...
1. **Resolve the thread and PR coordinates.** Read
   `./docs/pr/<pr>-active.json` (written by `pr-review-intake`). The
   file gives you both the thread metadata (`active[]` entry matching
   `--thread`) and the AzDO `context` block (`org`, `project`, `repo`,
   `pr_number`) needed to address the API. If the file is missing or
   the thread isn't in it, ask the user to re-run `pr-review-intake`
   first and stop. **Do not re-run `detect_context.sh`** — the
   persisted context is intake's snapshot, which is what we want even
   if the agent has switched branches.
...
4. **Post + transition.** For Azure DevOps:
   ...
   python3 scripts/update_azdo.py \
     ...
     --commits <sha,sha,...> \
     --summary '<verbatim text the agent supplies>'
   ```
   The script reads PR coordinates from `./docs/pr/<pr>-active.json`
   `context` block and formats the reply body internally per
   `references/reply-templates.md`. Agent's only inputs are the flags
   above. Use `--trivial` instead of `--summary` only with
   `--action pending` for typo-grade fixes.

   Auth: `AZURE_DEVOPS_EXT_PAT` MUST be exported (write scope:
   "Pull Request Threads (read & write)"). If it isn't, the script
   exits with `azdo_not_logged_in` before any HTTP call.
